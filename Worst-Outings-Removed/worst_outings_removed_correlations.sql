...
NextYear AS (
	SELECT 
		playername,
		SUBSTRING(date, 1, 4) AS nextYear,
		FORMAT((9 * SUM(CAST(p_er AS FLOAT))) / (SUM(CAST(p_ipouts AS FLOAT)) / 3), 'N2') AS nextERA
	FROM 
		pitching
	JOIN
		bios
...
INNER JOIN
	NextYear
ON
	AllGames.playername = NextYear.playername
AND
	AllGames.year + 1 = NextYear.nextYear
ORDER BY
	RankedSum.playername, RankedSum.year